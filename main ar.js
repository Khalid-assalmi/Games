...
function add() {
    ...
    button.textContent = "إضافة الاسم";
    button.id = "click";
    ...
    button.onclick = function() {
        ...
        if (inp.value.trim() != "") {
            ...
            let inp = document.getElementById("inp");
            sh.textContent = "خطأ: المدخلات فارغة";
...
function Show() {
    let num = 0;
    if (list.length <= 0) {
        dis.innerHTML = "<div class='Empty'><h3>أنت لا تملك أي أسماء.</h3></h3><button onclick='add()'>إضافة اسماء</button></div>";
    } else {
...
function run() {
    ...
    var repeat = setInterval(() => {
        ...
        show.innerHTML = `<div><a>${list[com]}</a></div>`+ `<div class='bottom'><button onclick='back()'>إغلاق</button></div>`;
        }, 150);
    setTimeout(() => {
...
function edit(index) {
    ...
    sh.className = "sh";
    let button = document.createElement("button");
    button.textContent = "تعديل الاسم";
    button.id = "click";
    document.body.appendChild(div);
    div.appendChild(btn);
    div.appendChild(h3);
